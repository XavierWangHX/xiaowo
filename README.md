...
- **LLM API 代理**：在 AI 客户端和实际 LLM API 之间插入代理，拦截请求时并行生成 6 路注入内容（记忆碎片/房间场景/音乐/时间/反刍/常驻记忆），组装进 system_prompt 后转发
...
4. 编辑 `config.json`，填入 API 密钥
5. 启动服务端：`node index.js`（自动拉起 ChromaDB，监听 3456 端口）
6. （可选）启动代理：代理随服务端一起启动，监听 3457 端口
7. 浏览器打开 `http://localhost:3456` 查看前端界面
...
每次 AI 客户端发起请求时，代理并行调用 6 路小窝 API：

| 注入源 | 端点 | 注入区域 |
|--------|------|---------|
| 常驻记忆 | 直读 `data/persistent/` | `## 常驻记忆` |
| 项目摘要 | 直读 `data/projects/` | `## 记忆位置说明` |
| 生成式召回 | `/api/v2/recall` | `## 浮现` |
| 时间流逝 | `/api/v2/time-passage` | `## 时间经过` |
| 房间场景 | `/api/app` room | `## 我看到的 / 我感受到的 / 我听到的` |
| 音乐状态 | `/api/app` music | `## 音乐` |
| 反刍（5%） | 随机读 `rumination/` | `## 反刍` |

同时加载固定前缀（`前缀.md`），让 AI 知道自己住在小窝里。

### 双格式支持

代理同时支持 OpenAI 和 Anthropic 两种 API 格式，按请求路径自动识别：

| 客户端请求路径 | 格式 | System prompt 位置 |
|---------------|------|-------------------|
| `/v1/chat/completions` | OpenAI | `messages[]` 中 `role: "system"` |
| `/v1/messages` | Anthropic | 顶层 `system` 字段 |

流式响应（SSE）完整透传，不做任何修改。
...
将 AI 客户端的 API 地址改为 `http://localhost:3457/v1`，API key 随便填（代理会用自己的 key 转发）。

```
Claude Code / 其他框架
    → http://localhost:3457/v1/chat/completions
    → 代理拦截 → 注入记忆/场景/音乐 → 转发到实际 LLM API
```
...
## MCP 部署（Claude Code 用户）

MCP 让 Claude Code 直接用自然语言操作你的记忆系统。

### 本地部署

1. 确保小窝服务在跑（`cd server && node index.js`）
2. 编辑 Claude Code 的设置（项目目录下 `.claude/settings.json` 或全局设置）：
...
```json
{
  "mcpServers": {
    "xiaowo": {
      "command": "node",
      "args": ["你的小窝路径/server/mcp-server.js"]
    }
  }
}
```

3. 重启 Claude Code，之后可以直接说"写一条事件记忆：今天做了什么"。
